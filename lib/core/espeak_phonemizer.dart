...
class EspeakPhonemizer {
  ...
  static Future<String> _extractDataIfNeeded() async {
    ...
    // Load the zip -- try PAD file path first, fall back to Flutter assets
    final Uint8List zipBytes;
    final padPath = await AssetManager.espeakDataPath;
    if (!padPath.startsWith('assets/') && await File(padPath).exists()) {
      zipBytes = await File(padPath).readAsBytes();
    } else {
