...
class _ReaderScreenState extends State<ReaderScreen> with WidgetsBindingObserver {
  ...
  static String _voiceDisplayName(String voiceId) {
    // Format: {region}{gender}_{name}  e.g. "bm_lewis", "af_heart"
    // Some voices may be just a prefix with no name, e.g. "af"
    ...
    if (parts.length < 2 || parts[1].isEmpty) {
      // No name part -- just show flag + gender (e.g. "US Female")
      return '$flag $gender'.trim();
    }

    final rawName = parts.sublist(1).join(' ');
  ...
  Future<void> _initializeTts() async {
    ...
    try {
      // Ensure PAD assets are copied to files dir (no-op if already done)
      await AssetManager.ensureAssets(
        onProgress: (asset, current, total) {
          debugPrint('[Reader] Preparing asset $current/$total: $asset');
        },
      );

      await _ttsEngine.initialize(
        modelPath: await AssetManager.modelPath,
        voicesPath: await AssetManager.voicesPath,
        isInt8: false,
      );
