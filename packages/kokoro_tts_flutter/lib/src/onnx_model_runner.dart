...
class OnnxModelRunner {
  ...
  List<String> _getPreferredProviders() {
    ...
    try {
      ...
      if (isInt8) {
        ...
        // Handle nested arrays (batch dimension) - flatten if needed
        final List<double> outputList = _flattenToDoubles(rawList);
  ...
  /// Flattens a potentially nested list to a flat list of doubles.
  /// Handles cases where ONNX returns Float32List, List<double>, or nested arrays.
  List<double> _flattenToDoubles(List<dynamic> rawList) {
    ...
    void flatten(dynamic item) {
      if (item is double) {
        ...
        // Try to convert to double as last resort
        try {
          result.add((item as num).toDouble());
        } catch (e) {
          debugPrint('Warning: Could not convert $item to double, skipping');
        }
      }
    }
