...
class FlutterOnnxruntimePlugin : FlutterPlugin, MethodCallHandler {
    ...
    // Background scope for running ONNX inference off the main thread
    ...
    override fun onMethodCall(
        ...
        when (call.method) {
            ...
            "runInference" -> {
                // Validate arguments on the main thread (fast)
                ...
                // Resolve OrtValue references on the main thread (fast, just HashMap lookups)
                ...
                // Build run options on the main thread (fast)
                val ortRunOptions =
                        ...
                        if (runOptions.containsKey("logSeverityLevel")) {
                            ...
                            val logLevel =
                                when (level) {
                                    ...
                                    else -> OrtLoggingLevel.ORT_LOGGING_LEVEL_WARNING
                                }
                ...
                // *** HEAVY WORK: Run inference on a background thread ***
                // This is the key change -- session.run() takes 5-7 seconds for TTS
                // and previously blocked the Android UI thread, freezing the entire device.
                inferenceScope.launch {
                    try {
                        // Run ONNX inference on Dispatchers.Default (background thread pool)
                        val ortOutputs =
                        ...
                        // Process outputs (still on background thread -- just HashMap/UUID ops)
                        ...
                        // Return result on the main thread (required by Flutter method channel)
    ...
    override fun onDetachedFromEngine(
        ...
        // Cancel any pending inference coroutines
